...
interact(".medicine-item").draggable({
...
interact(".dropzone-patient").dropzone({
  ...
  ondrop: function (event) {
    ...
    if (attr == currentmedic) {
      ...
      document
        .getElementById(currentProduct + "1")
        .querySelector(".medicImg1").style.opacity = "1";
      show.classList.add("d-none");
      var em = document
        .getElementById(currentProduct + "1")
        .querySelector(".medicImg1");

      elementOpacity = window.getComputedStyle(em).getPropertyValue("opacity");
      var descripTxt = document
        .getElementById(currentProduct + "1")
        .querySelector("#des-txt");
    ...
    if (medicTarget !== undefined) {
      medicines.forEach((element) => {
        if (element.img === medicTarget) {
          ...
          var em = document
            .getElementById(show.id + "1")
            .querySelector(".medicImg1");
          var elementOpacity = window
            .getComputedStyle(em)
          ...
          if (elementOpacity != 1) {
            ...
            if (!productResult.className.includes("d-none")) {
              ...
              dropped1.classList.add(
                "animate__animated",
                "animate__rollOut",
                "animate__slow"
              );
              dropped2.classList.add(
                "animate__animated",
                "animate__rollOut",
                "animate__slow"
              );
              setTimeout(function () {
                dropped1.classList.remove(
                  "animate__animated",
                  "animate__rollOut",
                  "animate__slow"
                );
      ...
      (one == "amino" || one == "hypromelose" || one == "lipoic") &&
      (two == "amino" || two == "hypromelose" || two == "lipoic") &&
      array.length > 1
    ) {
      document.getElementById("warn-div").style.display = "flex";
      var myStyle = document.getElementById("warn-div").offsetLeft;

      if (myStyle > 0 && medicTarget == undefined && array.length > 2) {
